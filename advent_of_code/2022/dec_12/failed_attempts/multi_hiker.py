...
class HeightMap:
    ...
    def process_map_data(self, raw_map_data: list[str]) -> list[list[MapPosition]]:
        ...
        for y, row in enumerate(raw_map_data):
            heightmap_row = [MapPosition(x, y, ord(char), char) for x, char in enumerate(row)]
    ...
    def get_distance_from_position_to_target(self, position: Position) -> int:
        ...
        return abs(position.x - self.ending_position.x) + abs(position.y - self.ending_position.y)
...
class Hiker:
    ...
    def begin_journey(self) -> None:
        ...
        while True:
            ...
            if not num_moves:
                ...
                return
            if num_moves == 1 and not self.move_to_position(paths[0]):
                return
            if num_moves > 1:
                logging.info(
    ...
    def move_to_position(self, position: Position) -> bool:
        ...
        logging.info(f"{self.name} was on an inefficient path and stopped at {position}")
        return False
