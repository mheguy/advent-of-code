...
class Hiker:
    ...
    def get_best_move(self) -> Position | None:
        ...
        if possible_moves := set(possible_moves) - set(self.visited_positions):
            return min(possible_moves, key=self.heightmap.get_distance_from_position_to_target)
        return None

    def undo_step(self) -> None:
        print(f"--- Backtracking: We have no way forward. Moving from {self.position} to {self.path_history[-1]}!!!")
...
def get_number_of_steps(hiker: Hiker) -> int:
    ...
    while True:
        if best_move := hiker.get_best_move():
            if result := hiker.move_to_position(best_move):
                if not least_steps or result < least_steps:
                    print(f"Found a new shortest route: {result} steps! Previous was: {least_steps}")
                    least_steps = result
                hiker.undo_step()
                hiker.undo_step()
        else:
        ...
        if hiker.position == hiker.heightmap.starting_position and len(hiker.visited_positions) > 4:
            return least_steps
