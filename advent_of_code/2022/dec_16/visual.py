...
def create_neighbors_graph(valve_map) -> None:
    dot = graphviz.Graph("dec_16_raw", comment="AOC Dec 16", directory=INPUT_FOLDER, format="png")
    edges = set()
...
def create_routes_graph(valve_map: dict[str, Valve]) -> None:
    dot = graphviz.Graph("dec_16_abbreviated", comment="AOC Dec 16", directory=INPUT_FOLDER, format="png")
    edges = set()
    for name, valve in valve_map.items():
        if not valve.routes:
            continue
