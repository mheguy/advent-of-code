...
class Display:
    ...
    def __repr__(self) -> str:
        return (
            "\n".join(["".join(self.display_matrix[i]) for i in range(len(self.display_matrix))])
...
def main():
    ...
    output = "\n".join(["".join(cpu.display.display_matrix[i]) for i in range(len(cpu.display.display_matrix))])
    print(output)
