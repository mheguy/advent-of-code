...
class HeightMap:
    def __init__(self, raw_map_data: list[str]) -> None:
        self.starting_position = self._get_and_mask_special_position(raw_map_data, "S", "a")
        self.ending_position = self._get_and_mask_special_position(raw_map_data, "E", "z")
        self.x_max = len(raw_map_data[0]) - 1
        self.y_max = len(raw_map_data) - 1
        self.map_data = self.process_map_data(raw_map_data)

    @staticmethod
    def _get_and_mask_special_position(raw_map_data: list[str], char: str, new_char: str) -> Position:
    ...
    def get_departure_points(self) -> Generator[Position]:
        all_departure_points = (pos for pos, char in self.map_data.items() if char == "a")
        ...
        for departure_point in all_departure_points:
            ...
            surrounded_by_losers = all(point_char == self.map_data[adjacent] for adjacent in adjacents)
...
def main(lines: list[str]) -> None:
    ...
    steps_in_shortest_path = min(Hiker(start, heightmap).explore_paths() for start in heightmap.get_departure_points())
